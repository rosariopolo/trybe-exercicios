...
const warriorAttackDamage = (warrior) => {
...
const gameActions = {
  warriorTurn: (warriorAttackDamage) => {
    const warriorDamage = warriorAttackDamage(warrior);
...
// Ela será a função que simula o turno do personagem mage. Essa HOF receberá como parâmetro a função que calcula o dano deferido pelo personagem mage e atualizará os healthPoints do monstro dragon. Além disso, ela também deve atualizar o valor das chaves damagee mana do mage.
  mageTurn: (mageAttackDamage) => {
    const mageTurnStats = mageAttackDamage(mage);
...
gameActions.warriorTurn(warriorAttackDamage);
gameActions.mageTurn(mageAttackDamage);
